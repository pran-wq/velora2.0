        ...
        <header className="flex justify-between items-center py-6 md:px-8 relative z-20">
          ...
          <div className="flex items-center gap-6">
            <div className="relative w-full max-w-[320px] hidden lg:block">
              ...
              <input type="text" placeholder="Search metrics, insights, reports..." className="w-full pl-10 pr-6 py-3 bg-white rounded-full shadow-sm border border-gray-100 text-sm outline-none placeholder:text-gray-400 transition-all focus:ring-2 focus:ring-slate-300/30" />
            </div>
            <div className="relative" ref={reminderRef}>
              ...
              >
                ...
                <div className="absolute -top-1 -right-1 w-5 h-5 bg-slate-700 rounded-full border-2 border-[#F8FAFC] flex items-center justify-center text-[10px] text-white font-bold">3</div>
              </div>
                ...
                {showReminders && (
                  ...
                  >
                     <div className="p-4 border-b border-gray-50 flex justify-between items-center bg-[#F8FAFC]">
                        ...
                        <span className="text-[10px] text-slate-500 font-bold uppercase cursor-pointer hover:underline">Clear All</span>
                     </div>
                     <div className="max-h-80 overflow-y-auto p-2">
                        {[
                          { t: 'Vitamin D3', s: 'Take 1 capsule', time: '8:00 AM', c: '#6366f1' },
                          { t: 'Cardiology Follow-up', s: 'Appt in 2 days', time: 'May 12', c: '#4f46e5' },
                          { t: 'Log Evening Hydration', s: 'Remaining: 800ml', time: '7:00 PM', c: '#0d9488' }
                        ].map((r, i) => (
                          <div key={i} className="p-3 hover:bg-[#F8FAFC] rounded-xl flex gap-3 transition-colors cursor-pointer">
                             ...
                             <div>
                                ...
                                <div className="flex items-center gap-1 mt-1.5 text-[9px] font-bold text-slate-500">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-6 px-4 md:px-8 relative z-10 pb-16">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
               {[
                 { label: 'Set Reminders', icon: Bell, color: 'bg-teal-50 text-teal-600', path: '/reminders' },
                 { label: 'Book Appointment', icon: Calendar, color: 'bg-indigo-50 text-indigo-500', path: '/appointments' },
                 { label: 'Start Exercise', icon: Dumbbell, color: 'bg-slate-100 text-slate-700', path: '/workouts' },
                 { label: 'Nutrition Log', icon: Utensils, color: 'bg-teal-50 text-teal-600', path: '/nutrition' }
               ].map((action) => (
                 <button 
                   key={action.label}
            ...
            <div className="bg-white rounded-[32px] p-8 shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-gray-50 flex flex-col relative overflow-hidden">
               ...
               <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-8 items-center relative z-10">
                  ...
                  <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                    {[
                      { label: 'Energy', val: Math.min(100, Math.round(stats.recoveryScore * 0.9)), color: '#4f46e5', icon: Zap },
                      { label: 'Strength', val: Math.min(100, Math.round(stats.recoveryScore * 1.1)), color: '#6366f1', icon: Dumbbell },
                      { label: 'Recovery', val: stats.recoveryScore, color: '#0d9488', icon: ShieldPlus },
                      { label: 'Mind & Focus', val: Math.min(100, Math.round(stats.recoveryScore * 1.05)), color: '#818cf8', icon: Brain },
                    ].map(m => (
                  ...
                  <div className="relative w-64 h-64 flex items-center justify-center mx-auto">
                     ...
                     <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                        <span className="text-xs font-bold text-indigo-500 uppercase tracking-wider mb-1">Overall Score</span>
                        ...
                        <span className="text-lg font-bold text-slate-600">{stats.recoveryScore > 80 ? 'Optimal' : stats.recoveryScore > 70 ? 'Good' : 'Needs Rest'}</span>
                        <span className="text-[10px] text-slate-500 mt-1">{stats.recoveryScore > 80 ? 'Peak condition!' : 'Keep pushing forward!'}</span>
                     </div>
                     {/* Floating Icons */}
                     <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center text-indigo-500"><Zap size={14} fill="currentColor" /></div>
                    <div className="absolute right-0 top-[20%] w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center text-indigo-400"><Brain size={14} /></div>
                    <div className="absolute right-4 bottom-[15%] w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center text-teal-500"><ShieldPlus size={14} /></div>
                    <div className="absolute left-6 bottom-[10%] w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center text-indigo-400"><Moon size={14} fill="currentColor" /></div>
                    <div className="absolute left-[-0.5rem] top-[40%] w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center text-teal-500"><Dumbbell size={14} /></div>
                  </div>
                  ...
                  <div className="bg-[#F8FAFC] rounded-2xl p-6 border border-gray-100 flex flex-col justify-center h-full space-y-6">
                     <div>
                       ...
                       <div className="flex items-center gap-2">
                         <span className="text-2xl font-bold text-teal-600">Good</span>
                     ...
                     <div>
                       <div className="flex justify-between items-center mb-4">
                         ...
                         <span className="px-2 py-1 bg-teal-50 text-teal-600 rounded text-[10px] font-bold">↑ 8%</span>
                       </div>
                       {/* Real Sparkline / Trend Data */}
                       <div className="w-full h-12 flex items-end gap-1">
                          {stats.adherenceRate > 0 ? (
                            Array.from({ length: 7 }).map((_, i) => (
                              <div key={i} className="flex-1 bg-gradient-to-t from-teal-400 to-teal-300 rounded-t-sm opacity-80" style={{ height: `${Math.random() * 40 + 40}%` }} />
                            ))
                          ) : (
            ...
            <div className="space-y-6">
               <div className="flex justify-between items-center px-2">
                  ...
                  <button className="text-[9px] font-black text-teal-600 uppercase tracking-widest hover:underline">Manage Rx</button>
               </div>
               <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 -mx-2 px-2 snap-x snap-mandatory">
                  {[
                    { name: 'Magnesium Glycinate', dose: '400mg • 9 PM', status: 'TAKEN', bg: 'bg-teal-50/50', border: 'border-teal-100/60', color: 'text-teal-600' },
                    { name: 'Omega-3', dose: '1000mg • 8 AM', status: 'TAKEN', bg: 'bg-teal-50/50', border: 'border-teal-100/60', color: 'text-teal-600' },
                    ...
                    <div key={i} className={cn("flex-shrink-0 w-56 rounded-[28px] p-5 border shadow-sm transition-all snap-center", m.bg, m.border)}>
                       <div className="flex justify-between items-start mb-4">
                          <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm">
                             <Pill size={16} className={m.status === 'TAKEN' ? 'text-teal-600' : 'text-gray-400'} />
                          </div>
                          <button className={cn("px-3 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest shadow-sm", m.status === 'TAKEN' ? 'bg-white text-teal-600 border border-teal-200' : 'bg-[#0F172A] text-white')}>
                             {m.status}
                          </button>
                       </div>
                       <div>
            ...
            {/* Sleep Quality — wide horizontal card */}
            <div className="bg-white rounded-[24px] p-5 md:p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-gray-50 flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-10">
              <div className="flex items-center gap-4 shrink-0">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-500">
                  <Moon size={22} />
                </div>
                <div>
                  <span className="text-xs font-bold text-[#0F172A] block">Sleep Quality</span>
              ...
              <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-end gap-4">
                <div className="flex-1 w-full h-10 flex items-end gap-1.5 justify-between">
                  {[40, 60, 30, 80, 50, 70, stats.sleepHours * 10].map((v, i) => (
                    <div key={i} className="flex-1 max-w-[14px] mx-auto w-full bg-gradient-to-t from-indigo-400 to-indigo-300 rounded-full" style={{ height: `${Math.min(100, v)}%`, opacity: v > 60 ? 1 : 0.4 }} />
                  ))}
                </div>
                <div className="flex shrink-0 items-center justify-between sm:flex-col sm:items-end sm:gap-2 gap-4">
                  <span className="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-[10px] font-bold whitespace-nowrap">
                    {stats.sleepHours >= 7 ? 'Good' : 'Low'}
                  </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               ...
               <div className="bg-white rounded-[24px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-gray-50 flex flex-col justify-between">
                  ...
                  <div className="flex items-center gap-4">
                     <div className="relative w-20 h-20 shrink-0">
                       <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                          ...
                          <circle cx="50" cy="50" r="40" fill="none" stroke="#334155" strokeWidth="8" strokeDasharray="251" strokeDashoffset={251 - (251 * stats.recoveryScore) / 100} strokeLinecap="round" />
                       </svg>
                  ...
                  <button
                    ...
                    className="w-full mt-6 py-3 bg-slate-50 text-slate-600 font-bold text-xs rounded-xl hover:bg-slate-100 transition-colors border border-gray-100"
                  >
                    Open prediction hub
                  </button>
               </div>
               ...
               <div className="bg-white rounded-[24px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-gray-50 flex flex-col justify-between relative overflow-hidden min-h-[220px]">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-slate-200/30 rounded-full blur-3xl -z-0" />
                  ...
                  <div className="relative z-10 h-full flex flex-col">
                    <div className="flex items-center justify-between mb-4">
                       ...
                       <span className="px-2 py-0.5 bg-slate-200/30 text-slate-500 text-[9px] font-black uppercase rounded-full tracking-wider">Active Analysis</span>
                    </div>
          ...
          <div className="flex flex-col gap-6">
             ...
             <div className="bg-white rounded-[24px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-gray-50">
               <div className="flex items-center gap-2 mb-6">
                 <div className="w-8 h-8 rounded-full bg-slate-200/30 flex items-center justify-center text-slate-500"><Activity size={16} /></div>
                 <h3 className="font-bold text-[#0F172A] text-[15px]">Today's Focus</h3>
               </div>
               <div className="space-y-4">
                  {[
                    { title: 'Hit Protein Goal', val: '160 / 180 g', icon: Utensils, color: 'text-slate-600', bg: 'bg-slate-100' },
                    { title: 'Workout', val: 'Upper Body Strength', icon: Dumbbell, color: 'text-slate-600', bg: 'bg-slate-100' },
                    { title: 'Hydration', val: '2.1 / 3 L', icon: Droplets, color: 'text-slate-600', bg: 'bg-slate-100' },
                    { title: 'Sleep', val: '7.2 / 8 hr', icon: Moon, color: 'text-slate-500', bg: 'bg-slate-400/10' },
                  ].map(f => (
               ...
               <button className="w-full mt-6 text-slate-500 font-bold text-xs hover:underline">
                 View Full Plan
               </button>
             </div>
             ...
             <div className="bg-white rounded-[24px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-gray-50">
               <div className="flex items-center justify-between mb-6">
                 <div className="flex items-center gap-2">
                   <div className="w-8 h-8 rounded-full bg-teal-50 flex items-center justify-center text-teal-600"><Utensils size={16} /></div>
               ...
               <div className="space-y-4">
                  {[
                    { label: 'Protein', current: 160, goal: 180, color: 'bg-teal-500' },
                    { label: 'Carbs', current: 200, goal: 250, color: 'bg-indigo-500' },
                    { label: 'Fats', current: 65, goal: 70, color: 'bg-slate-400' },
                  ].map((macro) => (
             ...
             <div className="bg-white rounded-[24px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-gray-50">
               ...
               <div className="flex items-center justify-between group cursor-pointer hover:bg-[#F8FAFC] p-2 -mx-2 rounded-xl transition-colors">
                  <div className="flex items-center gap-3">
                     <div className="w-10 h-10 rounded-xl bg-[#F8FAFC] flex items-center justify-center text-slate-500">
                        <Dumbbell size={16} />
                     </div>
                     <div>
