...
import { 
  ...
  ArrowRight, History, ScanLine, CheckCircle2, Search, Trash2, Edit3, Image,
...
export default function FemaleProfile() {
  ...
  return (
    <div className="flex flex-col gap-10 pt-10 sm:pt-16 px-4 md:px-8 pb-36 max-w-[1400px] mx-auto overflow-y-auto no-scrollbar h-full w-full bg-[#FFFBFB]">
          ...
          {activeModal && (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 pointer-events-auto">
              <motion.div 
                ...
                className="absolute inset-0 bg-[#1F111F]/60 backdrop-blur-md"
              />
              <motion.div 
                  ...
                  <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-1">
                        <h3 className="text-2xl font-display font-bold text-[#1F111F] tracking-tight">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-rose-500">Identity Protocol</p>
                      </div>
                    </div>
                    <button 
                      onClick={() => setActiveModal(null)} 
                    ...
                    >
                      <X size={20} className="text-[#8E7E8E] group-hover:text-rose-600 transition-colors" />
                    </button>
                  </div>

                  <div className="space-y-4">
                    {activeModal === 'personality' && [
                      { id: 'Empathetic', label: 'The Empathetic', desc: 'Warm, nurturing & supportive' },
                      { id: 'Analyst', label: 'The Analyst', desc: 'Data-driven & clinical precision' },
                      { id: 'Stoic', label: 'The Stoic', desc: 'Calm, focused & direct' },
                      { id: 'Caretaker', label: 'The Caretaker', desc: 'Gentle & attentive guidance' },
                    ].map(opt => (
                      <button 
                        key={opt.id}
                        ...
                        className={cn(
                          ...
                          personality === opt.id ? "border-rose-600 bg-rose-50/30" : "border-gray-50 hover:border-rose-100 bg-gray-50/50"
                        )}
                      >
                        <div>
                          <p className="text-base font-bold text-[#1F111F] group-hover:text-rose-600 transition-colors">{opt.label}</p>
                          <p className="text-xs text-[#8E7E8E] font-medium mt-1">{opt.desc}</p>
                        </div>
                        {personality === opt.id && (
                          <div className="w-8 h-8 rounded-full bg-rose-600 flex items-center justify-center text-white shadow-lg shadow-rose-200">
                            <Check size={16} />
                          </div>
                        )}
                      </button>
                    ...
                    {activeModal === 'theme' && (
                      <div className="space-y-8 py-4">
                         <div className="grid grid-cols-2 gap-6">
                            <button onClick={() => showNotify('Light theme active')} className="aspect-square rounded-[2.5rem] bg-white border-2 border-rose-600 flex flex-col items-center justify-center gap-4 shadow-xl shadow-rose-100/50">
                               <div className="w-12 h-12 rounded-2xl bg-rose-50" />
                               <span className="text-xs font-black uppercase tracking-widest text-[#1F111F]">Light</span>
                            </button>
                            <button onClick={() => showNotify('Dark theme available in Premium')} className="aspect-square rounded-[2.5rem] bg-[#1F111F] border-2 border-transparent flex flex-col items-center justify-center gap-4 text-white hover:scale-105 transition-transform">
                               <div className="w-12 h-12 rounded-2xl bg-[#2D2D2D]" />
                    ...
                    {activeModal === 'account' && (
                      <div className="space-y-8">
                         <div className="space-y-6">
                            <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-rose-500 pl-1">Identity Vault</h4>
                            <div className="space-y-5">
                              <div className="space-y-2">
                                 <label className="text-[10px] font-black uppercase text-[#8E7E8E] pl-4 tracking-widest">Full Name</label>
                                 <input type="text" defaultValue={profile.name} className="w-full bg-gray-50 border border-gray-100 rounded-[1.5rem] p-5 outline-none font-bold text-[#1F111F] focus:border-rose-300/30 focus:bg-white transition-all shadow-inner" />
                              </div>
                              <div className="space-y-2">
                                 <label className="text-[10px] font-black uppercase text-[#8E7E8E] pl-4 tracking-widest">Email Address</label>
                                 <input type="email" defaultValue={profile.email || 'user@aether.health'} className="w-full bg-gray-50 border border-gray-100 rounded-[1.5rem] p-5 outline-none font-bold text-[#1F111F] focus:border-rose-300/30 focus:bg-white transition-all shadow-inner" />
                              </div>
                              <div className="space-y-2">
                                 <label className="text-[10px] font-black uppercase text-[#8E7E8E] pl-4 tracking-widest">Phone Number</label>
                                 <input type="tel" defaultValue="+91 98765 43210" className="w-full bg-gray-50 border border-gray-100 rounded-[1.5rem] p-5 outline-none font-bold text-[#1F111F] focus:border-rose-300/30 focus:bg-white transition-all shadow-inner" />
                              </div>
                            </div>
                         </div>
                         <div className="space-y-4 pt-8 border-t border-gray-50">
                            <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-rose-500 pl-1">Security Protocol</h4>
                            <button onClick={() => showNotify('Password reset email sent')} className="w-full p-6 rounded-[1.5rem] bg-gray-50 border border-gray-100 text-left flex items-center justify-between group hover:bg-rose-50 transition-all">
                               <span className="font-bold text-[#1F111F] group-hover:text-rose-600">Reset Access Password</span>
                               <ChevronRight size={18} className="text-[#8E7E8E] group-hover:text-rose-600 transition-all" />
                            </button>
                         </div>
                         <button onClick={() => { showNotify('Profile updated successfully'); setActiveModal(null); }} className="w-full py-6 bg-[#1F111F] text-white rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] mt-4 shadow-2xl shadow-black/10 hover:bg-rose-600 transition-all active:scale-95">
                            Commit Changes
                         </button>
                      </div>
                    )}
                    ...
                    {activeModal === 'notifications' && (
                      <div className="space-y-4">
                         {[
                           ...
                           { label: 'Hormonal Insights', enabled: true },
                         ].map((opt, i) => (
                           <div key={i} className="flex items-center justify-between p-6 rounded-[2rem] bg-gray-50 border border-gray-100 hover:bg-rose-50 transition-colors group">
                              <span className="font-bold text-[#1F111F] group-hover:text-rose-600 transition-colors">{opt.label}</span>
                              <div className={cn("w-12 h-6 rounded-full p-1 transition-all shadow-inner", opt.enabled ? "bg-rose-600" : "bg-gray-200")}>
                    ...
                    {activeModal === 'language' && (
                      <div className="space-y-4">
                         {['English (US)', 'Hindi (Beta)', 'Spanish', 'German'].map(lang => (
                           <button key={lang} onClick={() => showNotify(`Language set to ${lang}`)} className="w-full p-6 rounded-[2rem] border-2 border-gray-50 hover:border-rose-100 text-left font-bold text-[#1F111F] flex items-center justify-between group bg-gray-50/50">
                              <span className="group-hover:text-rose-600 transition-colors">{lang}</span>
                              {lang === 'English (US)' && <Check size={20} className="text-rose-600" />}
                           </button>
                         ))}
                      </div>
                    )}
                    ...
                    {activeModal === 'privacy' && (
                      <div className="space-y-10 text-center py-8">
                         ...
                         <div className="space-y-3">
                           <h4 className="text-2xl font-black text-[#1F111F] tracking-tight">Zero-Knowledge Privacy</h4>
                           <p className="text-sm text-[#8E7E8E] font-medium px-6 leading-relaxed">
                             Aether uses end-to-end encryption for all health records. Only you can access your biometric data.
                           </p>
                         </div>
                         <div className="pt-6 space-y-4">
                            <button className="w-full py-5 rounded-[1.5rem] bg-[#1F111F] text-white font-black uppercase tracking-widest text-[10px] shadow-xl shadow-black/10">Export My Vault</button>
                      ...
                      <div className="space-y-8">
                         <div className="p-8 rounded-[2.5rem] bg-gray-50 border border-gray-100 shadow-inner">
                            <div className="flex justify-between mb-4">
                               <span className="text-[11px] font-black uppercase tracking-widest text-[#8E7E8E]">Cloud Storage</span>
                               <span className="text-[11px] font-black text-rose-600">840 MB / 5 GB</span>
                            </div>
                            <div className="w-full h-3 bg-white rounded-full overflow-hidden shadow-sm border border-gray-100">
                               <div className="w-[18%] h-full bg-rose-600 rounded-full shadow-lg" />
                            </div>
                            <p className="text-[10px] text-[#8E7E8E] font-medium mt-6 text-center">Auto-syncing with Aether Health Vault</p>
                         </div>
                         <button onClick={() => showNotify('Cache cleared')} className="w-full py-6 rounded-[2rem] border-2 border-gray-50 hover:border-rose-100 font-black uppercase tracking-widest text-[10px] text-[#1F111F] transition-all">Optimize Storage</button>
                      </div>
                    )}

                    {activeModal === 'subscription' && (
                      <div className="space-y-8">
                         <div className="p-8 rounded-[3rem] bg-gradient-to-br from-[#1F111F] to-[#2D2D2D] text-white relative overflow-hidden shadow-2xl">
                            <div className="relative z-10">
                               <div className="flex justify-between items-start mb-10">
                                  <div>
                                     <p className="text-[11px] font-black uppercase tracking-[0.3em] text-rose-400 mb-2">Member Tier</p>
                                     <h4 className="text-4xl font-black tracking-tight">Rose PRO</h4>
                                  </div>
                                  <div className="px-4 py-1.5 bg-rose-600/20 rounded-full border border-rose-500/30 backdrop-blur-md">
                                     <span className="text-[11px] font-black uppercase tracking-widest text-rose-400">Premium</span>
                                  </div>
                               </div>
                               <div className="space-y-4 mb-10">
                                  {[
                                    ...
                                    <div key={f} className="flex items-center gap-3 text-sm font-bold text-gray-300">
                                       <div className="w-5 h-5 rounded-full bg-rose-600/20 flex items-center justify-center text-rose-600">
                               ...
                               <div className="flex items-center justify-between pt-8 border-t border-white/10">
                                  <span className="text-[10px] font-black uppercase tracking-widest text-[#8E7E8E]">Subscription ID: ATH-R884</span>
                                  <span className="text-xl font-black text-white">ACTIVE</span>
                               </div>
                            </div>
                            <Sparkles className="absolute -bottom-10 -right-10 p-4 opacity-10 text-rose-600 scale-[2] rotate-12" size={150} />
                         </div>
                         <div className="grid grid-cols-2 gap-4">
                            <button className="py-5 rounded-[1.5rem] bg-gray-50 border border-gray-100 font-black uppercase tracking-widest text-[10px] text-[#1F111F] hover:bg-rose-50 hover:border-rose-100 transition-all">Billing</button>
                            <button className="py-5 rounded-[1.5rem] bg-rose-50 border border-rose-100 font-black uppercase tracking-widest text-[10px] text-rose-600 hover:bg-rose-100 transition-all">Gift Plan</button>
                         </div>
                      </div>
                    )}
                    ...
                    {activeModal === 'help' && (
                      <div className="space-y-8">
                         <div className="relative">
                            ...
                            <input type="text" placeholder="Search Help Vault" className="w-full bg-gray-50 border border-gray-100 rounded-[2rem] py-6 pl-16 pr-8 outline-none focus:border-rose-300/30 focus:bg-white font-bold text-sm text-[#1F111F] shadow-inner" />
                         </div>
                         <div className="grid grid-cols-2 gap-4">
                            {[
                              { label: 'Technical Help', icon: Smartphone, color: 'text-indigo-500', bg: 'bg-indigo-50' },
                              { label: 'Cycle Advisory', icon: Droplets, color: 'text-rose-500', bg: 'bg-rose-50' },
                              { label: 'Billing Support', icon: CreditCard, color: 'text-emerald-500', bg: 'bg-emerald-50' },
                              { label: 'General FAQ', icon: HelpCircle, color: 'text-amber-500', bg: 'bg-amber-50' },
                            ].map(item => (
                              <button key={item.label} className="p-6 rounded-[2.5rem] bg-white border border-gray-100 shadow-sm hover:shadow-xl hover:border-rose-100 transition-all text-left group">
                                 ...
                                 <p className="font-black text-[13px] text-[#1F111F] leading-tight group-hover:text-rose-600 transition-colors">{item.label}</p>
                              </button>
                            ))}
                         </div>
                         <button className="w-full py-6 bg-rose-600 text-white rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl shadow-rose-100 hover:scale-[1.02] active:scale-95 transition-all">
                    ...
                    {activeModal === 'about' && (
                      <div className="space-y-12 py-8 text-center">
                         <div className="relative inline-block">
                           <div className="w-32 h-32 bg-gradient-to-tr from-rose-500 to-rose-400 rounded-[3.5rem] flex items-center justify-center shadow-2xl shadow-rose-100 relative z-10">
                         ...
                         <div className="space-y-2">
                           <h4 className="text-4xl font-black text-[#1F111F] tracking-tighter">Aether Health</h4>
                           <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#8E7E8E]">Intelligence for Life • v2.4.0</p>
                         </div>
                         ...
                         <div className="space-y-3">
                            {[
                              ...
                              <button key={link.label} className="w-full p-6 rounded-[1.5rem] bg-gray-50 flex items-center justify-between group hover:bg-rose-50 transition-all border border-transparent hover:border-rose-100">
                                 <div className="flex items-center gap-4">
                                    <link.icon size={22} className="text-[#8E7E8E] group-hover:text-rose-600 transition-colors" />
                                    <span className="font-bold text-sm text-[#1F111F] group-hover:text-rose-600">{link.label}</span>
                                 </div>
                                 <ChevronRight size={18} className="text-[#8E7E8E]" />
                              </button>
                            ))}
                         </div>
                         
                         <div className="pt-6 space-y-2">
                           <p className="text-[10px] text-[#8E7E8E] font-black uppercase tracking-[0.5em]">
                              Aether Technologies
                           </p>
      ...
      <AnimatePresence>
        {notification && (
          <motion.div 
            ...
            className="fixed bottom-12 left-1/2 -translate-x-1/2 z-[100] px-6 py-4 bg-[#1F111F] text-white rounded-3xl shadow-2xl flex items-center gap-4 border border-white/10 backdrop-blur-xl"
          >
      ...
      <section className="space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            <h1 className="text-4xl md:text-6xl font-display font-black text-[#1F111F] tracking-tighter">
              Hello, <span className="text-rose-500">{firstName}</span>.
            </h1>
            <div className="flex items-center gap-3">
              <div className="w-2 h-2 rounded-full bg-rose-500 animate-pulse" />
              <p className="text-sm text-[#8E7E8E] font-bold uppercase tracking-widest">
                Dynamic Daily Status: <span className="text-[#1F111F]">Cycle Day {stats.cycleDay || 14} • High Fertility</span>
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4 mb-8">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-white rounded-2xl shadow-sm border border-rose-100 flex items-center justify-center text-rose-500">
              <Heart size={28} />
            </div>
            <div>
              <h2 className="text-3xl font-black text-[#1F111F] tracking-tight">Health Card</h2>
              <p className="text-sm text-[#8E7E8E] font-medium">Your basic health identity</p>
            </div>
          </div>
        </div>
        ...
        <section className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          ...
          <div className="xl:col-span-12 h-full mt-4">
            <button 
              ...
              className="w-full bg-white rounded-[3rem] p-8 text-[#1F111F] relative overflow-hidden group shadow-sm border border-rose-100 hover:bg-rose-50 transition-all flex items-center justify-between"
            >
              <div className="flex items-center gap-8">
                ...
                <div className="text-left">
                  ...
                  <p className="text-sm text-[#8E7E8E] font-medium leading-relaxed">
                    Connect with your AI wellness companion for instant support.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 text-rose-600 font-black text-[12px] uppercase tracking-[0.2em] px-8 py-4 bg-rose-50 rounded-2xl group-hover:bg-rose-600 group-hover:text-white transition-all">
              ...
              <div className="absolute top-0 right-1/4 p-8 opacity-[0.03] group-hover:scale-110 transition-transform">
                <Brain size={120} className="text-rose-600" />
              </div>
            </button>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-8 mt-10">
          {settingsGroups.map((group, i) => (
            <div key={i} className="space-y-6">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-rose-500 pl-2">{group.title}</h3>
                ...
                {group.items.map((item, j) => (
                  ...
                  >
                    <div className="flex items-center gap-5">
                      <div className="w-12 h-12 rounded-2xl bg-slate-50 text-slate-500 flex items-center justify-center group-hover:bg-rose-50 group-hover:text-rose-600 transition-colors">
                        <item.icon size={22} />
                      </div>
                      <div className="text-left">
                        <h4 className="text-sm font-black text-[#1F111F]">{item.label}</h4>
                        <p className="text-[11px] text-[#8E7E8E] font-medium">{item.desc}</p>
                      </div>
                    </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-10">
          <div className="p-8 sm:p-10 rounded-[3rem] bg-rose-50 border border-rose-100 space-y-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-rose-600 tracking-tight">SOS Protocols</h3>
                  <p className="text-[9px] font-black text-rose-400 uppercase tracking-widest mt-0.5">Response Active</p>
                </div>
              </div>
              <button className="px-5 py-2.5 rounded-full bg-white text-rose-600 font-black text-[9px] uppercase tracking-widest border border-rose-100 shadow-sm hover:bg-rose-600 hover:text-white transition-all">
                Setup SOS
              </button>
            </div>

            <div className="space-y-4">
              <div className="p-5 rounded-3xl bg-white border border-rose-100 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-rose-50 text-rose-500 flex items-center justify-center">
                    <Phone size={20} />
                  </div>
                  <div>
                    <p className="text-[9px] font-black text-rose-400 uppercase tracking-widest">Mom (Primary)</p>
              ...
              <div className="space-y-3 pt-4 border-t border-rose-100">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-white/50 border border-rose-100/50">
                    <div>
                      <h4 className="text-[13px] font-bold text-rose-700">{toggle.label}</h4>
                      <p className="text-[9px] text-rose-400 font-medium">{toggle.desc}</p>
                    </div>
                    <button className="w-10 h-5 rounded-full bg-rose-500 relative p-1 shadow-inner">
          ...
          <div className="flex flex-col">
            ...
            >
              <div className="w-20 h-20 rounded-[2rem] bg-slate-50 flex items-center justify-center group-hover:bg-rose-500 group-hover:text-white transition-all duration-500">
                <LogOut size={32} />
              </div>
              <div className="text-center">
                <h3 className="text-xl font-black uppercase tracking-[0.2em] mb-2">De-Authorize</h3>
                <p className="text-xs font-medium text-[#8E7E8E] group-hover:text-rose-400">Sign out and purge local cache</p>
              </div>
            </button>
          </div>
        </section>
